...
public class PolicyOutdoorPropertySearchReport : SearchReportEngine
{
	private bool m_test;
	public PolicyOutdoorPropertySearchReport(string agentes, bool test) :
		base(agentes)
	{
		m_test = test;
	}
	...
	protected override IEnumerable<SearchResult> SearchById(string id)
	{
		...
		try
		{
			var res = db.spList_SearchByPolicyNo(id, m_agentes, ApplicationConstants.COMPANY).ToList();
			var result = res.Where(p => !string.IsNullOrEmpty(p.outdoorDescription)).Single();
			...
			return new SearchResult[] {
				new SearchResult {
					...
					outdoorDescription = result.outdoorDescription,
					outdoor = true
				}
			};
		}
	...
	protected override IEnumerable<SearchResult> SearchLikePolicyNo(string policyNo)
	{
		...
		try
		{
			var results = db.spList_SearchByPolicyNo(policyNo, m_agentes, ApplicationConstants.COMPANY).ToList();

			var finalResult =
				results
				.Where(p => !string.IsNullOrEmpty(p.outdoorDescription))
				.Select(m =>
			{
				...
				return new SearchResult
				{
					...
					outdoorDescription = m.outdoorDescription,
					outdoor = true
				};
			});
	...
	protected override IEnumerable<SearchResult> SearchByName(string name)
	{
		...
		try
		{
			var results = db.spList_SearchPolicyByClientName(name, m_agentes, ApplicationConstants.COMPANY).ToList();
	...
	protected override IEnumerable<SearchResult> SearchByDates(DateTime start, DateTime end)
	{
		...
		try
		{
			var results = db.spList_SearchPolicyByIssueDate(start, end, m_agentes, ApplicationConstants.COMPANY).ToList();
	...
	protected override IEnumerable<SearchResult> SearchByAgent(string agent)
	{
		...
		try
		{
			var results = db.spList_SearchPolicyByUser(int.Parse(agent), m_agentes, ApplicationConstants.COMPANY).ToList();
	...
	protected override IEnumerable<SearchResult> SearchByExpiring(DateTime start, DateTime end, bool includeRenewed)
	{
		...
		try
		{
			var results = db.spList_SearchPolicyByExpiringDate(start, end, m_agentes, ApplicationConstants.COMPANY, m_test, includeRenewed).ToList();
